...
const Customize = () => {
  ...
  const getNextStatus = (currentStatus, serviceType = 'customization') => {
  ...
  const getNextStatusLabel = (currentStatus, serviceType = 'customization') => {
                  ...
                  <tr key={item.item_id} className="clickable-row" onClick={() => handleViewDetails(item)}>
                    ...
                    <td onClick={(e) => e.stopPropagation()}>
                      {item.approval_status === 'pending_review' || item.approval_status === 'pending' || item.approval_status === null || item.approval_status === undefined || item.approval_status === '' ? (
                        ...
                        <div className="action-buttons">
                          {getNextStatus(item.approval_status, 'customization') && (
                            <button 
                              className="icon-btn next-status" 
                              onClick={() => updateStatus(item.item_id, getNextStatus(item.approval_status, 'customization'))} 
                              title={`Move to ${getNextStatusLabel(item.approval_status, 'customization')}`}
