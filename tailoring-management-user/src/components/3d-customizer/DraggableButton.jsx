...
export default function DraggableButton({ id, modelPath, position, color, scale = 0.15, onPositionChange, onSelect, isSelected, onMovingChange }) {
    ...
    // Helper function to check if an object is a child of the button group
    ...
    useFrame(() => {
        if (isMoving && groupRef.current) {
            ...
            // Find the first hit that is NOT this button itself
    ...
    const handleClick = (e) => {
        ...
        if (!isMoving) {
            // First click - pick up the button
            setIsMoving(true);
            onSelect(id);
            gl.domElement.style.cursor = 'move';
            if (onMovingChange) onMovingChange(true);
        } else {
            // Second click - place the button
            setIsMoving(false);
            if (groupRef.current) {
